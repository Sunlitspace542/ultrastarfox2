## Note
Star Fox 2 uses the ARGSFX assembler. The documentation for it is currently lost.  
Its syntax is based on that of SASM, the assembler used for Star Fox. SASM's manual has been included here for reference.  
Note that not everything mentioned in the following document applies to ARGSFX, and ARGSFX includes some extra directives that SASM does not have.  
